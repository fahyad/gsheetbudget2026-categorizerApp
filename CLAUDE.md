...
## Current versions
- **Apps Script:** v11.17 — Phase 2 of time-driven email parsing: `handleParseAndFetch_` is now read-only by default. Inline Gmail scan is opt-in via `?withParse=1`, sent only by the Parse pill / empty-state Refresh button (the explicit "I want fresh now" paths). Mount-time auto-refresh skips the parse, dropping server work from ~1-3s to ~200ms while the hourly trigger keeps the sheet fresh in background. Backward compat: old PWAs that don't pass `withParse` get the read-only path; old Apps Script (pre-v11.17) ignores the new param and parses anyway. v11.16 added the time-driven trigger (Phase 1) — `processInfoAlertsTrigger` + `installEmailTrigger`/`uninstallEmailTrigger` menu items, hourly auto-parse. v11.15 made `buildBudgetDashboard_` auto-set Budget B1 to the period containing today's date on every refresh (helper `currentPeriodLabel_(setup)`). v11.14 added `handleArchiveGoal_`/`handleUnarchiveGoal_` (Saving goal archive; recovered via `clasp pull` 2026-04-26). v11.13 added `_elapsedMs` echo + `logClientMetrics` action + auto-created `ClientMetrics` tab. v11.12 fixed Budget `#REF!` via `rebuildBudgetInternal_` delegation; v11.9–v11.11 were Saving tab shakedown.
