...
## Current versions
- **Apps Script:** v11.14 — adds `handleArchiveGoal_` + `handleUnarchiveGoal_` endpoints for archiving/unarchiving Saving tab goals. **v11.14 was deployed from another machine/session — source recovered into this repo via `clasp pull` on 2026-04-26; full doc entries for v11.14 are TODO.** v11.13 added `_elapsedMs` echo + `logClientMetrics` action + auto-created `ClientMetrics` tab. v11.12 fixed Budget `#REF!` via `rebuildBudgetInternal_` delegation; v11.9–v11.11 were Saving tab shakedown.
