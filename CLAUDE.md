...
## Current versions
- **Apps Script:** v11.16 — adds time-driven email-parsing trigger (Phase 1 of the time-driven parsing redesign). New `processInfoAlertsTrigger` handler + `installEmailTrigger`/`uninstallEmailTrigger` menu items under Budget Tools. Hourly auto-parse so new transactions appear in the sheet without opening the PWA. Trigger uses the existing `processInfoAlerts_` (LockService-protected since v11.3). Logs only "interesting" runs (parsed > 0 OR errors > 0) to keep Logs tab quiet. PWA contract unchanged — `parseAndFetch` still parses inline as fallback. **One-time setup:** owner must (a) deploy this version, (b) manually run `processInfoAlertsTrigger` from the Apps Script editor to force any auth re-grant, (c) Budget Tools → Setup Email Trigger. v11.15 made `buildBudgetDashboard_` auto-set Budget B1 to the period containing today's date on every refresh (helper `currentPeriodLabel_(setup)`). v11.14 added `handleArchiveGoal_`/`handleUnarchiveGoal_` (Saving goal archive; recovered via `clasp pull` 2026-04-26). v11.13 added `_elapsedMs` echo + `logClientMetrics` action + auto-created `ClientMetrics` tab. v11.12 fixed Budget `#REF!` via `rebuildBudgetInternal_` delegation; v11.9–v11.11 were Saving tab shakedown.
