...
- **PWA:** v0.19.6 (cache v36) on `main` — pixel UI is the canonical theme as of 2026-05-09 (graduated from `pwa/pixel-ui-redesign` via merge commit; the dedicated branch is preserved on remote as a snapshot but not active). Force-pixel via the early `<head>` script in `index.html`; no in-app toggle (removed in v0.19.1). Mono CSS in `style.css` stays as the structural foundation pixel.css overrides — bringing the toggle back is a single-commit revert (the v0.19.1 commit). v0.19.6 added the FIXED summary-cell accordion: tap to expand a sticky panel of fixed expenses due in the selected period. v0.19.3 paired with Apps Script v11.19 — `parseDashboard` reads `row[6]` as available (col G shifted by v11.19), `row[5]` as new `rolledOver` field. Cache key bumped to invalidate pre-v11.19 cached data. v0.19.0 added Phase G multi-select category rail (arm a chip → check N transactions → batch commit; falls back to today's tap-to-pick when no chip armed). v0.18.0 → v0.18.2 introduced theme infrastructure + dashboard polish + period-bar today-chip + terminal calendar styling. v0.17.0 was Phase 2 of time-driven email parsing. v0.16.0 introduced persistent views; v0.15.4 was cold-start optimization.
