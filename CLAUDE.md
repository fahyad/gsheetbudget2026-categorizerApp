...
## Current versions
- **Apps Script:** v11.18 — Goal archive flow: new `Archive Goal...` / `Unarchive Goal...` menu items wrap the existing (previously dormant) `handleArchiveGoal_` endpoint, force a Budget rebuild, and disclose the data-loss trade-off in a confirmation prompt. `rebuildBudgetInternal_` now reads Setup `D2:F100` and filters archived sub-categories — symmetric to `handleCategories_` and addresses the user's "Banff still in Budget after archive" report. `handleArchiveGoal_` / `handleUnarchiveGoal_` were refactored into lockless internals + thin web-handler wrappers so the menu can hold one lock across archive + rebuild. Duplicate-goal-name detection added — fails fast if the same goal name appears in multiple Saving rows. v11.17 Phase 2 of time-driven email parsing — `handleParseAndFetch_` is now read-only by default; inline Gmail scan is opt-in via `?withParse=1` (sent only by the Parse pill / empty-state Refresh). Mount-time refresh ~200 ms server vs ~1–3 s with parse. v11.16 added the trigger (Phase 1) — `processInfoAlertsTrigger` + `installEmailTrigger`/`uninstallEmailTrigger` menu items, hourly auto-parse. v11.15 made `buildBudgetDashboard_` auto-set Budget B1 to today's period via `currentPeriodLabel_(setup)`. v11.14 added the (then-dormant) goal archive endpoints. v11.13 added `_elapsedMs` echo + `logClientMetrics` + `ClientMetrics` tab. v11.12 fixed Budget `#REF!` via `rebuildBudgetInternal_` delegation; v11.9–v11.11 were Saving tab shakedown.
- **PWA:** v0.19.1 (cache v31) — branch `pwa/pixel-ui-redesign`, NOT yet merged to main (main at v0.17.0 / cache v26). Phase 25 redesign — terminal/pixel UI overlay scoped via `:root[data-theme="pixel"]` in `css/pixel.css`. Per-branch the theme is force-set to pixel by the early `<head>` script in `index.html`; the toggle was removed in v0.19.1 after on-device testing showed mono had drifted on the branch (see Phase 25 status). Mono CSS in `style.css` stays as the structural foundation pixel.css overrides — bringing the toggle back is a single-commit revert. v0.19.0 added Phase G multi-select category rail (arm a chip → check N transactions → batch commit; falls back to today's tap-to-pick when no chip armed). v0.18.0 → v0.18.2 introduced theme infrastructure + dashboard polish + period-bar today-chip + terminal calendar styling. v0.17.0 was Phase 2 of time-driven email parsing — `parseAndFetch({ withParse })` defaults read-only, mount-time refresh pairs the trigger's eventual consistency with persistent-views cold-open. v0.16.0 introduced persistent views; v0.15.4 was cold-start optimization.
