...
## Current versions
- **Apps Script:** v11.19 — Budget tab gets a new **Rolled Over** column (col F) between Spent and Available. Surfaces the prior-period Available carryover that was previously hidden inside the Available formula — explains cases like "Groceries: $0.00 LEFT" with "$0 spent / $98 budget" (answer: $98 of overspend rolled in from prior period). Available formula simplifies to `=F + D - E` (pure arithmetic); the SUMIFS lookup of prior-period Available now lives in `buildRolledOverFormula_` (col F). New `Budget_RolledOver` named range; `Budget_Available` named range moves col F → col G. Slicer + currency formatting updated for the new 7-col layout. **PWA needs paired update** to read row[6] as available (currently reads row[5]); v0.19.3 PWA fix is queued and will deploy after the sheet update is verified. v11.18 added Goal archive flow (Archive Goal / Unarchive Goal menu items + Setup col F filter in rebuildBudgetInternal_). v11.17 Phase 2 of time-driven email parsing — `handleParseAndFetch_` read-only by default. v11.16 added the trigger (Phase 1). v11.15 made `buildBudgetDashboard_` auto-set Budget B1 to today's period via `currentPeriodLabel_(setup)`. v11.14 added the (then-dormant) goal archive endpoints. v11.13 added `_elapsedMs` echo + `logClientMetrics` + `ClientMetrics` tab. v11.12 fixed Budget `#REF!` via `rebuildBudgetInternal_` delegation; v11.9–v11.11 were Saving tab shakedown.
- **PWA:** v0.17.0 (cache v26) — Phase 2 partner of v11.17. `js/api.js` `parseAndFetch({ withParse })` defaults to read-only; `force: true` paths (Parse pill in the period bar, empty-state Refresh) pass `withParse: true` so the user's explicit "fresh now" actions still scan Gmail. Mount-time auto-refresh just reads — pairs the trigger's eventual consistency with the perceived-instant cold open from v0.16.0 persistent views. v0.16.0 (cache v25) introduced persistent views — the router keeps view containers mounted and toggles `hidden` on tab switches instead of throwing away DOM, making tab navigation instant after first mount. v0.15.4 was cold-start optimization (dedupe `categories`, defer suggest index, persist transactions cache, throttle silent re-mount). Branch: `main`.
