...
## Current versions
- **Apps Script:** v11.15 — `buildBudgetDashboard_` now auto-sets Budget B1 to the period containing today's date on every refresh (was hardcoded to first period; PWA Dashboard reads B1 → wrong period silently rendered wrong data after every Update Script). New helper `currentPeriodLabel_(setup)` reads PayPeriods and returns today's label. v11.14 added `handleArchiveGoal_` + `handleUnarchiveGoal_` endpoints for archiving/unarchiving Saving tab goals (recovered via `clasp pull` 2026-04-26; full v11.14 doc entries TODO). v11.13 added `_elapsedMs` echo + `logClientMetrics` action + auto-created `ClientMetrics` tab. v11.12 fixed Budget `#REF!` via `rebuildBudgetInternal_` delegation; v11.9–v11.11 were Saving tab shakedown.
