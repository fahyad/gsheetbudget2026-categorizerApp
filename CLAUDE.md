...
## Current versions
- **Apps Script:** v11.25 — Phase 33: Reports tab removed (~500 LOC across `buildReportsTab_` / `applyReportsStructure_` / `buildReportsFromFormula_` / `buildReportsToFormula_` / `buildFixedExpensesFormulaByRange_`). User adopted native Google Sheets slicers on the Transactions tab (filter chips for Category + Period) as a simpler replacement for the Reports tab's filter+drill-down UX — covers the common-case "show me txns in Category X / Period Y" without custom formulas, with built-in Sheets sort/filter affordances. The v11.24 blank-`FixedExpenses_DueDay` guard in `buildFixedExpensesFormula_` (used by Budget tab) is preserved — it was a real bug fix for the Budget tab independent of Reports. v11.24 — Phase 32 (bug-fix pass): (1) Fixed Expenses formula now handles blank cells in `FixedExpenses_DueDay` (rows 16-50 are unused but cause `DATE(year, m, "")` to throw `#VALUE!` which propagated through SUMPRODUCT → outer IFERROR returned $0). Bug also existed in original `buildFixedExpensesFormula_` used by Budget tab — silent there because most periods don't contain a day-1; fix applied to both. (2) New "Uncategorized" row at row 50 of Reports tab surfaces transactions with empty Category in the selected date range — previously counted in Total Spent but invisible in the breakdown, causing math to look wrong by the uncategorized total. v11.23 — Phase 32: new **Reports** tab — spending breakdown by category for any time frame (Today / This Week / Last 7 Days / This Month / Last Month / This Period / Last Period / YTD / Last Year / All Time / Custom) with drill-down to per-transaction list. Single tab, formula-driven, three zones (Controls / Summary / Drill-down). INCOME section + SPENDING BY CATEGORY (sorted by Spent desc, zero-spend hidden via FILTER) + single "Fixed Expenses (scheduled)" line for Fixed Monthly Expenses due in range. Sparklines on % of total via native `=SPARKLINE`; heat-map conditional formatting on Spent column. New helpers: `buildReportsTab_`, `applyReportsStructure_`, `buildReportsFromFormula_`, `buildReportsToFormula_`, `buildFixedExpensesFormulaByRange_`. Wired into `buildWorkbook` + `updateWorkbook` after `setNamedRanges_`. Takes effect via `Update Script` to render. v11.22 was Phase 31: Instructions tab refresh (content-only). Comprehensive update across TABS / MENU FUNCTIONS / FIRST-TIME SETUP / DAILY USE / SAVING GOALS / TROUBLESHOOTING / FOR DEVELOPERS sections — adds entries for `Archive Goal...`, `Unarchive Goal...`, `Setup Email Trigger`, `Remove Email Trigger`, `Dedupe + Normalize Transactions (rescue)` menu items + `ClientMetrics` tab + hourly auto-parse workflow + multi-select category rail + `Budget col G (Rolled Over)` warning. No behavioral change. Takes effect via `Update Script` menu on user's sheet. v11.21 was Phase 30: duplicate-parsing + period-bug fix. Three layered fixes: (1) write-time timestamp dedup in `processInfoAlerts_` catches Gmail thread re-iteration (when a new email arrives in an already-labeled thread, Gmail's negative-label search briefly re-matches → parser re-iterates ALL messages in thread → identical hash suffixes produced duplicate rows; visible damage: 15 duplicate groups, ~$435 over-counted spending); (2) `shortHash_` widened from 4 → 8 hex chars (2 → 4 MD5 bytes) so write-time dedup has zero false-drop risk from hash collision; (3) Period formula in `setTransactionFormulas_` now uses `INT(A{tr})` to truncate time portion when comparing against PayPeriods Start/End (fixes silent loss of last-day-of-period transactions to "Unassigned" — ~$165 lost from Budget totals in visible window). Parser also normalizes Date column to midnight (belt + suspenders with formula fix). New menu item `Dedupe + Normalize Transactions (rescue)` for one-shot cleanup of existing data. v11.20 was Phase 29: new `bootstrap` action returns categories + parseAndFetch in one call. Halves cold-mount round-trips in the PWA. v11.19 was Phase 27: Budget tab gets a new **Rolled Over** column (col F) between Spent and Available. Surfaces the prior-period Available carryover that was previously hidden inside the Available formula — explains cases like "Groceries: $0.00 LEFT" with "$0 spent / $98 budget" (answer: $98 of overspend rolled in from prior period). Available formula simplifies to `=F + D - E` (pure arithmetic); the SUMIFS lookup of prior-period Available now lives in `buildRolledOverFormula_` (col F). New `Budget_RolledOver` named range; `Budget_Available` shifts F → G. PWA paired with v0.19.3 (`parseDashboard` reads `row[6]` as available, `row[5]` as new `rolledOver` field). v11.18 added Goal archive flow (Archive Goal / Unarchive Goal menu items + Setup col F filter in `rebuildBudgetInternal_`). v11.17 Phase 2 of time-driven email parsing — `handleParseAndFetch_` read-only by default. v11.16 added the trigger (Phase 1) — `processInfoAlertsTrigger` + `installEmailTrigger`/`uninstallEmailTrigger` menu items, hourly auto-parse. v11.15 made `buildBudgetDashboard_` auto-set Budget B1 to today's period via `currentPeriodLabel_(setup)`. v11.14 added the (then-dormant) goal archive endpoints. v11.13 added `_elapsedMs` echo + `logClientMetrics` + `ClientMetrics` tab.
...
31. **`main` is force-pixel; no theme toggle in Settings.** Pixel UI graduated to main on 2026-05-09. The early `<head>` script in `index.html` hardcodes `data-theme="pixel"` before any CSS parses; no in-app way to switch back to mono — the toggle UI was removed in v0.19.1 after on-device feedback that mono had drifted (the multi-select rail, period bar today chip, etc. were designed for pixel; mono variants felt out of place once pixel was the daily driver). To revert visually: revert the v0.19.1 commit (restores the toggle in Settings + the localStorage-driven theme attribute) OR check out the `main-pre-pixel-merge` tag for the pre-merge state. Mono CSS in `style.css` stays as the structural foundation pixel.css overrides — bringing the toggle back is a single-commit revert. The `pwa/pixel-ui-redesign` branch is preserved on remote as a snapshot of the pre-merge state but is no longer active. See `docs/findings.md` "Pixel UI Theme System (PWA v0.18.0 → v0.19.1)".
...
33. **CSS `position: sticky` only floats within its containing block's bounds (v0.19.5).** A sticky element appears stuck while its parent's box is still on screen, then scrolls away with the parent once the parent's bottom passes the sticky offset. If the parent is shrink-to-fit (e.g., a single-row wrapper with no explicit height), the sticky child can only stick within those few pixels — past that, it scrolls off as if `position: static`. Symptom in v0.19.5: `.period-bar` had `position: sticky; top: 0` but its containing block `#period-bar-host` was a thin shrink-to-fit div, so the period bar disappeared as soon as the user scrolled ~60 px. **Fix:** promote sticky to the wrapper element (`#period-bar-host` got the sticky property + z-index 9) — the wrapper now has the full scroll-extent of the page to stick within. Same pattern applied to `.fixed-panel` in v0.19.6 (sticky stack ladder: period bar at z-index 9, fixed panel at z-index 6, top offset of `calc(env(safe-area-inset-top, 0px) + 108px)` so the panel sits below the period bar without overlap). When adding a new sticky element, audit its containing-block chain — the nearest ancestor with `position: relative/absolute/fixed` (or the initial containing block) defines the stick range, NOT the visual viewport. See `docs/findings.md` "FIXED Summary-Cell Accordion (v0.19.4 → v0.19.6)".

34. **Gmail's `-label:X` search has consistency lag — never rely on label exclusion alone for dedup (v11.21).** Scotiabank info-alerts all share the literal same subject ("Authorization on your credit account"), which makes Gmail bundle them into one giant thread. When a NEW message arrives in an already-labeled thread, Gmail's negative-label search briefly re-matches that thread (eventual consistency between message-add and label-inheritance). The parser's inner loop `for (m in allMessages[t])` then iterates ALL messages — including previously-processed ones — and produces byte-identical timestamps because the per-batch hash collision counter resets across runs. Visible damage in the user's sheet: 15 timestamp-duplicate groups, ~$435 of over-counted spending; ladder pattern (oldest message in a thread had the most duplicates because more messages arrived after it). **Fix:** ALWAYS dedup at the write site against existing sheet data — don't trust upstream exclusion to be perfect. Pattern in `processInfoAlerts_` Step 3.5: read all existing Timestamp values into a Set, filter newRows before setValues, log `dedup_skip` count to detect ongoing re-iteration. Defense in depth: widened `shortHash_` from 4 → 8 hex chars to eliminate the false-drop risk of hash collisions between different real transactions. See `docs/findings.md` "Duplicate Parsing + Period Bug (Phase 30)".

35. **Apps Script `Date` written to a sheet column keeps its time portion — formulas comparing against midnight Dates break on period boundaries (v11.21).** The parser previously wrote `emailDate` (full datetime, e.g. `May 12 2026 12:50:00`) to col A. The Period formula compared this against `PayPeriods End` stored as midnight Dates. On the LAST DAY of any period: `Setup!B (May 12 midnight) >= A (May 12 12:50)` → FALSE → no match → "Unassigned" → row silently dropped from Budget Spent totals. 100% of last-day-of-period transactions in the user's data were affected (~$165 lost across visible window). **Fix:** two layers — (1) parser normalizes Date column to midnight at write time using `new Date(year, month, day)`; (2) Period formula wraps the Date cell in `INT(...)` to truncate any time portion before comparing. Either alone would fix it; doing both means a future regression in either layer can't reintroduce the bug. When writing dates to sheet columns that participate in date-range formulas, always normalize to date-only OR wrap comparisons in `INT()`. See `docs/findings.md` "Duplicate Parsing + Period Bug (Phase 30)".
