...
- **Apps Script:** v11.25 — Phase 33: Reports tab removed; user adopted native Sheets slicers (Category + Period) on Transactions tab as a simpler replacement. v11.24 blank-`FixedExpenses_DueDay` guard preserved in `buildFixedExpensesFormula_` (benefits Budget tab independently). v11.24 — Phase 32 bug-fix pass. v11.23 — Phase 32: short-lived **Reports** tab — spending breakdown by category for any time frame (Today / This Week / Last 7 Days / This Month / Last Month / This Period / Last Period / YTD / Last Year / All Time / Custom) with drill-down to per-transaction list. Three zones (Controls / Summary / Drill-down). Income section + Spending sorted by Spent desc with zero-spend filtered out + single Fixed Expenses line. Native `=SPARKLINE` bars + heat-map conditional formatting. v11.22 was Phase 31: Instructions tab refresh (content-only update; no behavioral change). Brings the in-sheet Instructions content in sync with current state after 11 versions of drift since Phase 13b — adds `Archive Goal` / `Unarchive Goal` / `Setup Email Trigger` / `Remove Email Trigger` / `Dedupe + Normalize Transactions (rescue)` menu items + `ClientMetrics` tab + hourly auto-parse workflow + multi-select category rail + extended Budget cols (G = Rolled Over) warning. v11.21 was Phase 30: duplicate-parsing + period-bug fix. Two critical bugs discovered + fixed in one deploy. (1) Gmail thread re-iteration caused 15+ duplicate-timestamp groups in the user's Transactions tab (~$435 over-counted spending); fix is write-time timestamp dedup in `processInfoAlerts_` plus widened `shortHash_` (4 → 8 hex chars) to eliminate hash-collision false-drop risk. (2) Last-day-of-period transactions were silently lost to "Unassigned" period because Date col had time portion and PayPeriods End is midnight; fix is parser normalizes Date to midnight + Period formula wraps in `INT()` (belt + suspenders). New `Dedupe + Normalize Transactions (rescue)` menu item for one-shot cleanup of existing data. v11.20 was Phase 29: new `bootstrap` action returns categories + parseAndFetch in one call. Halves cold-mount round-trips in the PWA (one ~2.5 s 302+TLS network tax instead of two; projected ~7.3 s → ~4.8 s cold Categorize mount). Honors `withParse`; per-section error fields let the client recover from partial failures. Old endpoints stay live forever — bootstrap is purely additive. v11.19 was Phase 27: new "Rolled Over" column on Budget tab (col F, between Spent and Available). Available formula simplified to pure `F + D - E` arithmetic; the prior-period SUMIFS lookup extracted to `buildRolledOverFormula_`. New `Budget_RolledOver` named range; `Budget_Available` shifts F → G. PWA paired with v0.19.3 (parses row[6] as available). v11.18 added Goal archive flow (Archive Goal / Unarchive Goal menu items + rebuildBudgetInternal_ filters Setup col F). v11.17 Phase 2 of time-driven email parsing — `handleParseAndFetch_` skips inline Gmail scan unless `?withParse=1`. v11.16 added Phase 1 — `processInfoAlertsTrigger` + install/uninstall menu items, hourly auto-parse. v11.15 made `buildBudgetDashboard_` auto-set Budget B1 to the period containing today on every refresh. v11.14 added `handleArchiveGoal_`/`handleUnarchiveGoal_`. v11.13 added `_elapsedMs` echo + `logClientMetrics` + `ClientMetrics` tab. v11.12 fixed Budget `#REF!` cascade via `rebuildBudgetInternal_` delegation; v11.9–v11.11 were Saving tab shakedown. Full postmortems in `docs/findings.md`.
- **PWA:** v0.19.8 (cache v38) on `main` — Phase 29.2: cold Categorize mount uses the new `bootstrap` Apps Script action via `startBootstrap_` (single round-trip, falls back transparently to the v0.15.4 dual-fetch pattern on any failure). v0.19.7 (Phase 29.1) added two PWA-only quick wins: `<link rel="preconnect">` to script.google.com + script.googleusercontent.com warms TLS while HTML parses (~100-500 ms saved on first call), and cache-first paint on Dashboard cold open (peeks localStorage and renders immediately, then refreshes in background — turns the 5-7 s spinner into a <50 ms paint when cache exists). Pixel UI is the canonical theme as of 2026-05-09. Force-pixel via the early `<head>` script in `index.html`; no in-app theme toggle. v0.19.6 adds the FIXED summary-cell accordion. v0.19.3 paired with v11.19 — `parseDashboard` reads `row[6]` as available. Phase 25 — pixel UI redesign overlay (`css/pixel.css`) scoped via `:root[data-theme="pixel"]`. v0.19.0 added Phase G multi-select category rail. v0.18.0 → v0.18.2 introduced theme infrastructure + dashboard polish + period-bar today chip + terminal calendar styling. Earlier: v0.17.0 Phase 2 of time-driven email parsing; v0.16.0 persistent views; v0.15.4 cold-start optimization.
...
### Phase 29: Cold-Start Round 2 — Preconnect + Cache-First Dashboard + Bootstrap Endpoint (Apps Script v11.20 + PWA v0.19.7 → v0.19.8)
- **Status:** code complete + syntax-clean. **Awaiting Apps Script deploy** (user runs `./apps-script/deploy.sh "v11.20 — bootstrap"` from laptop). PWA changes auto-deploy on push. Detailed deploy + verification + rollback notes at the bottom of this Phase entry.

After the rulepop architecture comparison (2026-05-13) the user asked which of three perf proposals were worth implementing. Re-verified each against measured data (Phase 22 ClientMetrics: cold Categorize mount = 7,348-9,038 ms; per-call network tax = ~2,500 ms; period switch = pure client-side, no measured complaint). Three proposals validated, three skipped. Build step (Vite/esbuild) deferred — the bytes aren't the bottleneck (~50 KB gzipped already; the 7-9 s floor is round-trip count, not asset size) and adding a build step regresses the "edit + git push" workflow. CSS attribute period-filtering skipped — pre-rendering all 26 periods upfront would regress the cold mount we're trying to fix.

What shipped, in two PWA-side sub-versions plus one Apps Script bump:

- **v0.19.7 (Phase 29.1 — PWA-only wins, no Apps Script changes):**
  - **`<link rel="preconnect">`** to `script.google.com` and `script.googleusercontent.com` (the latter is the 302-redirect target for all Apps Script web app calls). Browser warms TLS while HTML parses; first API call after cold open finds the connection ready. Saves ~100-500 ms on the first call. Both hosts are stable and baked into the deployment ID, so the preconnect target won't drift.
  - **Cache-first paint on Dashboard cold open** (`js/views/dashboard.js mount()`): now uses `peekDashboardCache()` (added in v0.19.0 for the chip rail) to read localStorage synchronously, paint immediately if cache exists, then `load()` runs in the background. With a stale cache (older than the 10-min TTL) the previous behavior was to wait 5-7 s on a spinner; now it paints in <50 ms and refreshes silently. Same pattern as `store.transactions` got in v0.15.4.
  - Bumped APP_VERSION to v0.19.7, CACHE_VERSION to v37.

- **v11.20 (Phase 29.2 — Apps Script side):**
  - **New `handleBootstrap_` action** wraps `handleCategories_()` + `handleParseAndFetch_()` and returns the merged JSON. Both underlying handlers are independent (no shared locks, different tabs read), so combining them is purely additive on the server. Per-section error fields (`categoriesError`, `transactionsError`) let the client recover from partial failures without falling back. Echoes `_bootstrapMs` for client verification.
  - Old endpoints (`categories`, `parseAndFetch`) stay live forever — bootstrap is purely additive. Old PWAs continue to work unchanged.

- **v0.19.8 (Phase 29.2 — PWA side, requires v11.20 deployed first):**
  - **`api.bootstrap({withParse})`** in `js/api.js` — same surface as `parseAndFetch`, returns combined payload.
  - **`startBootstrap_({withParse})` helper** in `js/views/categorize.js` tries bootstrap first; on any failure (old Apps Script returning "Unknown action: bootstrap", network error, partial failure) falls back to the v0.15.4 `Promise.all([fetchCategories, parseAndFetch])` pattern transparently. Logs the fallback to console so we can detect bootstrap-endpoint health via `console.warn` filter.
  - **`bootstrapPromise` replaces `categoriesPromise`** as the module-level "in-flight pre-warm fetch" singleton. mount() fires it; refresh() awaits it on non-force, replaces it with a fresh `withParse:true` call on force.
  - Categories side-effect (`store.setCategories` + `renderCategories` + `renderChips` + `updateRailVisibility`) applied inside `startBootstrap_` as soon as data arrives — preserves the cold-mount UX where the rail populates before refresh completes.
  - Bumped APP_VERSION to v0.19.8, CACHE_VERSION to v38.

**Backward compat verification (per the architected plan):**
- Old PWA + new Apps Script: PWA never calls bootstrap, uses old endpoints. Works.
- New PWA + old Apps Script: PWA calls bootstrap, gets `{success:false, error:"Unknown action: bootstrap"}`, `request()` throws, fallback path fires. Slight first-call penalty (~2.5 s wasted on the failed bootstrap call), then normal. Acceptable for a deploy-window transient. Mitigation: deploy Apps Script BEFORE pushing the v0.19.8 PWA.
- New PWA + new Apps Script: bootstrap path used. ~2.5 s saved per cold mount.

**Expected impact (projected, requires real ClientMetrics measurement post-deploy to verify):**
- Cold Categorize mount: 7,348 ms → ~4,800 ms (~35% reduction)
- Cold Dashboard with stale cache: ~5-7 s spinner → ~50 ms paint + background refresh
- First-call latency on any view: -100 to -500 ms (preconnect)

**Rollback handles:**
- Phase 29.1 rollback: revert the v0.19.7 commit (single-file changes to index.html + dashboard.js + config.js + sw.js).
- Phase 29.2 PWA rollback: revert the v0.19.8 commit. Apps Script unchanged — keeps `bootstrap` action live but unused.
- Phase 29.2 Apps Script rollback: not needed (additive). If we did want to roll back, redeploy v11.19 — the new `bootstrap` action just becomes "Unknown action" and PWAs fall back to dual fetch.

- **Status:** code complete + syntax-clean. **Awaiting deploy** — Apps Script v11.20 must deploy from user's laptop (`./apps-script/deploy.sh "v11.20 — bootstrap endpoint"`) BEFORE PWA v0.19.8 ships, to avoid the 2.5 s deploy-window penalty. PWA changes auto-deploy on push to main. Verification post-deploy: ClientMetrics should show `bootstrap` rows with `Ok=true`, and `mount:categorize ClientTotalMs` ≈ 4,800 ms ± 1 s on cold sessions (down from 7,300+).

### Phase 30: Duplicate Parsing Fix + Period Boundary Fix (Apps Script v11.21)
- **Status:** complete + verified working (2026-05-17). Apps Script v11.21 deployed `@46`; user ran `Update Script` (formulas installed) + `Dedupe + Normalize Transactions (rescue)` (cleanup ran). Post-cleanup dumpSheet confirmed: 133 → 114 rows (19 phantom rows dropped, exact match to expected), 15 duplicate-timestamp groups → 0, 8 "Unassigned" rows → 0, all Date column values normalized to midnight. Cross-referenced against user's Scotiabank statement (Apr 7-30 billing cycle, 93 debit transactions): 61 exact matches + 3 date-offset matches (auth date ±1-2 days from post date — normal credit card behavior, not a bug). Statement-vs-PWA coverage gap of 29 transactions = $1,627 documented as "Statement vs PWA Coverage Gap" observation in findings.md (Scotiabank info-alerts don't fire for online/recurring/transit-app merchants — known upstream limitation, not in scope for Phase 30). No PWA changes — backend-only fix preserved existing PWA contract.

User noticed duplicate transactions in the PWA. Cross-referenced Gmail (source of truth, via newly-added Gmail connector) against the sheet (via dumpSheet endpoint with API key) and found TWO critical bugs:

**Bug 1 (CRITICAL):** Same email parsed multiple times across separate parser runs, producing identical timestamp rows. 15 distinct timestamp-duplicate groups in visible Transactions data; 19 phantom rows total; ~$435 of over-counted spending. **Root cause:** Scotiabank info-alerts all share the literal same subject ("Authorization on your credit account"), so Gmail bundles them into single threads. When a NEW message arrives in an already-`Budget/Processed`-labeled thread, Gmail's negative-label search briefly re-matches that thread (eventual consistency between message-add and label-inheritance). The parser's `for (m in allMessages[t])` inner loop then iterates ALL messages — including previously-processed ones — and the per-batch S3 hash collision counter resets across runs, producing byte-identical timestamps. Smoking gun: ladder pattern within a single Gmail thread (`19decb304f53f0f7`, May 3-4): 1st message had 4× duplicates, 2nd had 3×, 3rd had 2×, 4th had 1× — each new arrival re-parsed all prior messages.

**Bug 2 (CRITICAL, NEW DISCOVERY during investigation):** Transactions on the LAST DAY of any pay period silently went to `period='Unassigned'` and disappeared from Budget Spent totals. 8 of 8 last-day-of-period transactions affected in visible data (~$165 lost across visible window). **Root cause:** parser wrote `emailDate` (full datetime e.g. `May 12 2026 12:50:00`) to col A. Period formula compared this against `PayPeriods End` stored as midnight Dates: `Setup!B (May 12 midnight) >= A (May 12 12:50)` → FALSE → no match → "Unassigned". Mid-period transactions worked because period End was in the future; only last-day transactions failed the comparison.

**Bug 3 (LATENT, mitigated):** `shortHash_` used 4 hex chars (16 bits) = 65,536 possible values → birthday collision risk at ~256 distinct (merchant, amount) pairs. Not actively causing problems yet, but would become a false-drop risk under the new write-time dedup if two REAL different transactions hashed to the same value. Pre-emptively widened to 8 hex chars (32 bits, 4.3B values) to eliminate.

**Fixes shipped in v11.21 (single deploy, three layers):**

1. **`shortHash_` widened 4 → 8 hex chars** — eliminates Bug 3 (false-drop risk under new dedup). Backward compatible: existing rows with 4-char hashes still match by string equality.

2. **Write-time timestamp dedup in `processInfoAlerts_` Step 3.5** — reads existing Timestamps from col H before write, filters newRows to drop any timestamp already in sheet. Logs `dedup_skip` count so we can monitor whether Gmail thread re-iteration is still happening post-fix. Catches Bug 1 regardless of upstream label-exclusion working.

3. **Parser normalizes Date column to midnight** + **Period formula wraps cell in `INT()`** — fixes Bug 2 with defense in depth. Parser change handles all new writes (`new Date(emailDate.getFullYear(), emailDate.getMonth(), emailDate.getDate())`). Formula change in `setTransactionFormulas_` (`INT(A{tr})`) handles existing rows + any future regression in parser. Setup A/B values already midnight Dates — no INT() needed on Setup side.

4. **New menu item `Dedupe + Normalize Transactions (rescue)`** — one-shot cleanup script (`dedupeAndNormalizeTransactionsRescue`). LockService-protected. Groups data rows by Timestamp string, picks "best" row per group (prefers any non-empty Category; among multi-categorized, keeps first + warns), normalizes Date col to midnight on every kept row, atomic write via the existing consolidate-pattern (clear A/D/F/H → write back, leave E/G formulas alone). Confirmation prompt before destructive write. Safe to re-run (idempotent — no-ops on already-clean data).

**Edge cases handled:**
- Manual rows (Timestamp blank) pass through untouched — different identity rule from parsed rows.
- Multi-categorized duplicate groups (user fixed two siblings manually) — keep first, log warning so user can review.
- LockService prevents PWA writes during cleanup OR during parser dedup-then-write.
- Hash format change doesn't break PWA batchCategorize — matches by exact string equality regardless of suffix length.
- Existing rows still have time portion in Date column → fixed by either (a) cleanup script normalizing them, OR (b) new INT() formula making the formula tolerant. Both, in fact.
- `dedup_skip` log entries become a tripwire — if they keep firing post-fix, that means Gmail thread re-iteration is still actively happening and the dedup is doing its job.
- Manual entries with Date column having time portion: not normalized by cleanup (it only normalizes Dates already in Date format; manual user input may differ).

**Verification (Gmail-vs-Sheet cross-reference done before any code change):**
- 67 Gmail emails from last 30 days, ALL 67 present in sheet at least once → **no missing transactions** (data loss was over-counting only, never under-counting from emails)
- 15 timestamp-duplicate groups in sheet, ~$434.57 of phantom over-count
- 8 last-day-of-period 'Unassigned' rows, ~$165 stranded from Budget totals
- 0 lock timeouts in 500 logged entries → LockService working
- 0 parser errors in 27 trigger runs → regex parsing fine

**Rollback handles:**
- Apps Script: redeploy v11.20 (additive — new menu item becomes dangling reference but won't crash; new fixes revert; existing duplicates remain).
- Cleanup script: no auto-rollback (destructive by design — user is the safety check via the YES/NO confirmation). Sheet has version history via Google Sheets revision history if needed.

### Phase 31: Instructions Tab Refresh (Apps Script v11.22)
- **Status:** code complete + syntax-clean. Content-only update — no behavioral change. Awaiting `./apps-script/deploy.sh "v11.22 — instructions refresh"` + user runs `Update Script` to re-render the tab.

The in-sheet Instructions content was last comprehensively rewritten in Phase 13b (v10.3) and went stale across 11 Apps Script versions. Targeted refresh of seven sections in `buildInstructionsTab_` rows[] array — no structural change to the function, just content:

- **TABS:** added `ClientMetrics` (v11.13); dropped the "Pending removed" line (irrelevant since v11.0); noted v11.19 Rolled Over column on Budget tab.
- **MENU FUNCTIONS:** added `Archive Goal...` + `Unarchive Goal...` (v11.18), `Setup Email Trigger` + `Remove Email Trigger` (v11.16), `Dedupe + Normalize Transactions (rescue)` (v11.21). Reordered to match actual menu order in `onOpen`.
- **FIRST-TIME SETUP:** added Step 4 "Setup Email Trigger" so the hourly trigger gets installed during initial setup (previously a footgun — user could forget to install, then wonder why nothing parses).
- **DAILY USE:** rewrote to reflect (a) hourly trigger handles parsing automatically, PWA Refresh is read-only by default since v11.17; (b) Phase G multi-select category rail flow (arm chip → check N txns → Assign batch).
- **SAVING GOALS:** "When goal is achieved" rewritten — directs user to `Archive Goal...` menu (v11.18) instead of just "stop budgeting" (which leaves the tabs inconsistent per trip-up #29).
- **TROUBLESHOOTING:** added 3 entries — duplicate-rows fix path (run dedupe), last-day-of-period missing (v11.21 auto-fixed; run Update Script if old), trigger not running (run Setup Email Trigger once).
- **DO NOT:** updated Budget formula-col warning from "B, E, F" → "B, E, F, G" (v11.19 added Rolled Over at F, shifted Available to G). Noted that v11.21 now blocks duplicate creation even if user accidentally removes the `Budget/Processed` label.
- **FOR DEVELOPERS:** updated deploy command to recommend the `deploy "vXX"` shell alias (added in `scripts/deploy-alias.sh`); kept `./apps-script/deploy.sh` as fallback.

**Edge cases handled:** None — this is pure content. Rendering pipeline (`writeVersionBlock_` + protection) unchanged.

**Verification post-deploy:** open Instructions tab in sheet, eyeball that new menu items appear in the MENU FUNCTIONS table, that Dedupe entry appears at the bottom, that TROUBLESHOOTING has the dup/period/trigger entries.

**Rollback handles:**
- Trivial: redeploy any prior version. No data side effects. The protection on the tab (`setWarningOnly`) means user can also edit cells directly if a single line is wrong.

### Phase 32: Reports Tab (Apps Script v11.23 → v11.24 bug-fix pass)
- **Status:** v11.23 deployed `@48`, user ran Update Script + tested, 2 bugs found in real use. v11.24 fixes both. Awaiting `./apps-script/deploy.sh "v11.24 — reports fixed-expenses + uncategorized fixes"` + user runs Update Script again.

**v11.24 bug-fix pass (2026-05-17, post-deploy testing):**

User tested Reports tab with "Last Period" (Apr 29 - May 12). Two issues surfaced:

1. **Fixed Expenses showed $0.00** even though May 1 (when all 14 fixed monthly entries are due) falls in Apr 29 - May 12. Root cause: `FixedExpenses_DueDay` named range covers `C2:C50` (49 rows) but user has only ~14 entries; rows 16-50 are blank. The formula's `DATE(year, m, "")` for blank cells returns `#VALUE!`, which propagates through SUMPRODUCT, caught by outer IFERROR → returns 0 silently. **The same bug exists in the original `buildFixedExpensesFormula_` used by Budget tab** — silent there because most pay periods don't contain a day-1 (so $0 was correct by accident), but Apr 29 - May 12 DOES contain day-1 and would have been wrong too. Fix: coerce `ddRaw` to safe number via `IF(ddRaw="",1,IFERROR(ddRaw*1,1))`. Per-row `valid` multiplier still ensures blank rows contribute 0. Fix applied to BOTH formulas.

2. **Total Spent of $630.87 didn't match per-category breakdown** which summed to only $485.48 (gap = $145.39). Cause: 3 transactions in the period had empty Category column. Total Spent SUMIFS-on-all-negatives counted them, but the BYROW(CategoryList, LAMBDA) per-category breakdown iterates only configured categories from Setup → uncategorized txns invisible in the breakdown. Fix: new "Uncategorized" row at row 50 between Spending and Fixed. Always visible (shows $0 / 0 when nothing uncategorized — passive reminder). Amber color treatment (`#b45309`) and italic for visual differentiation. Fixed Expenses shifted to row 51, TOTAL to row 52.

**Files touched in v11.24:** `apps-script/Code.js` (2 helper formulas + Reports tab structure + CF rule range + F7 formula + layout doc comment). No PWA changes.

**Rollback handles for v11.24:** redeploy v11.23 (cleanup script not needed — just code change). Budget tab Fixed Expenses will revert to the silent-$0-for-day-1-containing-periods bug. Reports tab will lose Uncategorized row but still show data.

User wanted in-sheet reports: spending per category across configurable time frames (daily / weekly / per period / monthly / yearly etc) with drill-down to per-transaction list, including savings and fixed expenses unified. Planned by comparing 4 implementation approaches (single-tab formula-driven vs Apps Script generated vs multi-tab per time-frame vs hybrid-with-pivot); chose **Option A — single formula-driven tab** for live updates, custom date range support, low maintenance, and consistency with existing Budget/Saving tab patterns. User answered design questions: Q1a (Fixed Expenses as single line not per-merchant), Q2a (Savings treated like any other category), Q3b (Income as separate section above categories), defaults for Q4-Q10 (This Period default, two cells for Custom range, one-category-at-a-time drill, sort desc by Spent, hide zero-spend). Aesthetics confirmed via HTML mockup with real Apr 29-May 12 data before any code changes: A1a indigo tab color (#5c6bc0), A2a dark indigo section headers with white text, A3b bold larger Spent column, A4c native SPARKLINE bars, A5c heat-map gradient on Spent column, A6a drill-down matching Transactions tab style, A7a positive amounts.

**Tab structure (6 cols A-F, 88 rows):**
- **Rows 1-5 (Controls, frozen):** REPORTS header → Time-frame dropdown → From/To formula-driven cells → Custom range yellow-input cells (only used when "Custom" selected).
- **Rows 6-8 (Summary totals):** Total income | Total spent | Net | Total txns. Net shown in `$#,##0.00;[Red]-$#,##0.00` format so negative net auto-reds.
- **Rows 10-16 (Income section):** Per-positive-amount category, dynamic via `BYROW(CategoryList, LAMBDA)` + SUMIFS + FILTER + SORT. Green color treatment.
- **Rows 18-49 (Spending section):** Same pattern, ABS-summed for negative amounts, sorted by Spent desc. Up to 30 categories spill from row 20. Sparklines + percent in cols E/F pre-filled in row range.
- **Row 50:** Fixed Expenses (scheduled) — single line via `buildFixedExpensesFormulaByRange_($E$2, $E$3)`.
- **Row 51:** TOTAL row (gray bg).
- **Rows 54-88 (Drill-down):** Category dropdown → FILTER + SORT of Transactions matching category + date range. Spills up to 30 rows. Total row at 88.

**Time-frame dropdown** (11 options): Today, This Week, Last 7 Days, This Month, Last Month, This Period, Last Period, YTD, Last Year, All Time, Custom. From/To cells are formula-driven via `IFS` switch on B2; "Custom" reads from yellow input cells E4/E5.

**New helpers in `apps-script/Code.js`:**
- `buildReportsTab_(reports, ss)` — tab setup (color, widths, hide cols, freeze rows)
- `applyReportsStructure_(reports, ss)` — writes all formulas + formatting (~400 LOC)
- `buildReportsFromFormula_()` / `buildReportsToFormula_()` — date-range formula generators for E2/E3
- `buildFixedExpensesFormulaByRange_(fromCellRef, toCellRef)` — variant of existing `buildFixedExpensesFormula_` accepting raw From/To cell refs instead of a PayPeriods_Label

**Wired into:** `buildWorkbook` (creates Reports tab on first build) + `updateWorkbook` (rebuilds on Update Script). Both call `buildReportsTab_` AFTER `setNamedRanges_` because formulas reference `PayPeriods_*`, `CategoryList`, `CategoryMain`, `FixedExpenses_*`, `Transactions_*` named ranges. Instructions tab updated to list Reports in TABS section.

**Edge cases handled:**
- Zero-spend categories filtered out via `FILTER(combined, spent>0)` inside the LET.
- "Custom" with blank E4/E5 → defaults to TODAY()-30 / TODAY() so formula doesn't error.
- Conditional formatting on Spent column uses NUMBER (0) → PERCENTILE (100) gradient — robust to varying max spend across time frames.
- Drill-down with no matching transactions → IFERROR fallback shows "(no transactions in this category for the selected range)" message in A58.
- Drill-down with `B55` empty → category total at row 88 shows "(no category selected)" prefix.
- Conditional Net negative → red via number format `;[Red]-$...`.
- Tab protection: `setWarningOnly(true)` — user can edit cells if needed (no read-only enforcement).

**HTML mockup** built first (`reports-mockup.html`) using real Apr 29 - May 12 data from user's sheet, screenshotted via Claude Preview, user approved before code commit. Temp files (`reports-mockup.html` at project root, `.claude/launch.json` for the preview server) cleaned up at commit time — not committed.

**Verification post-deploy:**
1. Open sheet → `Budget Tools → 3. Update Script (safe)` → Reports tab appears in tab bar with indigo accent.
2. Click Reports → B2 dropdown defaults to "This Period" → From/To populate Apr 29-May 12 (or current period).
3. Income shows your manual paycheck entry. Spending shows variable categories sorted descending by Spent. Fixed Expenses row shows ~$2,334 (sum of all fixed monthly entries due in range). Sparklines + heat-map render correctly.
4. Drill-down: pick "Groceries" in B55 → 9 Groceries transactions in current period appear sorted newest first, total at row 88 = $200.47.
5. Change B2 to "YTD" → all numbers recompute live. Verify zero-spend categories don't appear.
6. Change B2 to "Custom" → type dates into E4/E5 → From/To at E2/E3 reflect the custom range.

**Rollback handles:**
- Apps Script: redeploy v11.22. New tab will be created but no longer get refreshed; can be deleted manually if undesired.
- Cleanup: delete Reports tab manually if rolling back. No data side effects elsewhere.

### Phase 33: Reports Tab Removed — Slicers Adopted Instead (Apps Script v11.25)
- **Status:** code complete + syntax-clean. Awaiting `./apps-script/deploy.sh "v11.25 — remove reports tab; slicers adopted"` + user runs `Update Script` (the prior code path would have recreated the Reports tab on each update — now removed).

User added native Google Sheets **slicers** to the Transactions tab (Category + Period filters via the green chip dropdowns visible at the top of the tab). The slicer-based filtering covers the common "show me transactions in Category X / Period Y" UX the Reports tab's drill-down provided — with less complexity, native Sheets affordances (sort/filter/export), and zero custom formulas to maintain. User deleted the Reports tab in the sheet and asked to remove the Apps Script plumbing so future `Update Script` runs don't recreate it.

**What was removed (v11.25):**
- `buildReportsTab_(reports, ss)` — tab setup (color, widths, freeze rows, hide cols)
- `applyReportsStructure_(reports, ss)` — ~400 LOC of formulas + formatting + CF rules + data validation
- `buildReportsFromFormula_()` / `buildReportsToFormula_()` — date-range formula generators using IFS switch on time-frame dropdown
- `buildFixedExpensesFormulaByRange_(fromCellRef, toCellRef)` — was only used by Reports; dead code now
- Call sites in `buildWorkbook` (`tabNames` array shed `'Reports'`, call to `buildReportsTab_` removed, alert text updated 7→6 tabs)
- Call sites in `updateWorkbook` (entire `--- Reports tab ---` block removed)
- Instructions tab TABS section: removed the "Reports — Spending by category for any time frame + drill-down (v11.23)" line

**What was kept:**
- `buildFixedExpensesFormula_` (with the v11.24 blank-dd guard) — used by Budget tab; the v11.24 fix was a real bug independent of Reports and stays.
- The two historical-context comments in `buildWorkbook` and the `buildFixedExpensesFormula_` docstring referencing Reports — left as breadcrumbs for future Claude sessions to understand why the helper exists.

**Side-effect on Budget tab:** unchanged. Budget tab's Fixed Expenses dashboard cell (B4) keeps the v11.24 fix; it now correctly computes for periods that contain a day-1 (was silently $0 pre-v11.24 even before Reports was added).

**Why slicers are better for this user's case:**
- Drill-down was the Reports tab's most-used feature. Slicers do it natively with chip UI.
- Per-category totals were the Reports tab's other use case. With slicers filtering Transactions, the built-in `SUMIF` formulas in Budget already show per-category totals per period; slicers reduce the "what's IN this category" question.
- The Reports tab's time-frame dropdown was novel but the user found Period-based slicer filtering covered ~90% of usage.
- Slicers also work without re-running Update Script — built-in Sheets feature, no Apps Script to maintain.

**Tradeoffs accepted:**
- No more arbitrary-date-range views (Custom From/To). User can use Sheets' built-in filter view if needed for ad-hoc ranges.
- No more sparkline + heat-map visualization. Trade for simpler maintenance.
- Fixed Expenses (scheduled) is no longer shown as a category-level total in a report context. Available via Budget tab B4.

**Rollback handles:**
- Apps Script: redeploy v11.24. Reports tab gets recreated on next Update Script run.
- Sheet: user-side — Reports tab can be recreated by deploying v11.24 + Update Script. Slicers stay on Transactions either way.
