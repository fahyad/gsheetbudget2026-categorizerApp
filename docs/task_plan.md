...
- **Apps Script:** v11.15 — `buildBudgetDashboard_` auto-sets Budget B1 to the period containing today on every refresh (was hardcoded to first period — PWA Dashboard reads B1, so silent wrong-period rendering after each Update Script). v11.14 added `handleArchiveGoal_`/`handleUnarchiveGoal_` (Saving goal archive endpoints; recovered via `clasp pull` 2026-04-26). v11.13 added `_elapsedMs` echo + `logClientMetrics` + `ClientMetrics` tab. v11.12 fixed Budget `#REF!` cascade via `rebuildBudgetInternal_` delegation; v11.9–v11.11 were Saving tab shakedown. Full postmortems in `docs/findings.md`.
