...
- **Status:** code complete + syntax-clean. **Awaiting deploy + cleanup run** on user's laptop. Deploy first (`./apps-script/deploy.sh "v11.21 — dup parsing + period bug fix"`), then user runs `Budget Tools → 3. Update Script (safe)` to install new Period formula across all 999 rows, then user runs `Budget Tools → Dedupe + Normalize Transactions (rescue)` once to clean up existing 15 duplicate-timestamp groups + 8 stranded "Unassigned" rows. No PWA changes — backend-only fix preserves existing PWA contract.
