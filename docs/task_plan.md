...
- **Apps Script:** v11.17 — Phase 2 of time-driven email parsing: `handleParseAndFetch_` skips the inline Gmail scan unless the caller passes `?withParse=1`. PWA mount-time refresh is now read-only (~200ms server vs ~1-3s when scanning); Parse pill + empty-state Refresh still force a scan. Hourly background trigger (v11.16) keeps the sheet fresh in either case. v11.16 added the trigger itself — `processInfoAlertsTrigger` + `installEmailTrigger`/`uninstallEmailTrigger` menu items. v11.15 made `buildBudgetDashboard_` auto-set Budget B1 to the period containing today on every refresh. v11.14 added `handleArchiveGoal_`/`handleUnarchiveGoal_` (Saving goal archive endpoints; recovered via `clasp pull` 2026-04-26). v11.13 added `_elapsedMs` echo + `logClientMetrics` + `ClientMetrics` tab. v11.12 fixed Budget `#REF!` cascade via `rebuildBudgetInternal_` delegation; v11.9–v11.11 were Saving tab shakedown. Full postmortems in `docs/findings.md`.
