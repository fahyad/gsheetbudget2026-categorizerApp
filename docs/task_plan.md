...
- **Apps Script:** v11.14 — adds `handleArchiveGoal_` + `handleUnarchiveGoal_` endpoints for archiving/unarchiving Saving tab goals (deployed from another session; source recovered via `clasp pull` 2026-04-26; full doc entries TODO). v11.13 added `_elapsedMs` echo + `logClientMetrics` + `ClientMetrics` tab. v11.12 fixed Budget `#REF!` cascade via `rebuildBudgetInternal_` delegation; v11.9–v11.11 were Saving tab shakedown. Full postmortems in `docs/findings.md`.
